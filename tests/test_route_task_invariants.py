...
class RouteTaskInvariantTests(unittest.TestCase):
    ...
    def test_only_confirmed_exits_complete_tasks(self):
        ...
        busy_state = (ROOT / "include" / "fsm" / "busy_exit_state.hpp").read_text(
            encoding="utf-8")
        self.assertIn('event == BusyExitEvent::COMPLETED', busy)
        self.assertIn('consecutiveMissing >= 3', busy_state)
    ...
    def test_obstacle_edits_only_in_path_and_slowdown_is_frame_scoped(self):
        ...
        core = read_runtime_sources()
        motion = (ROOT / "include" / "ctrl" / "motion.hpp").read_text(encoding="utf-8")
        guard = obstacle.index("if (obstacleRight <= leftColumn")
        truncation = obstacle.index("left.resize", guard)
    ...
    def test_fork_computed_sizes_and_gradients_are_guarded(self):
        ...
        self.assertNotIn("params->track->handle(true", source)
        collect = source.index("repair0.push_back(plannedRight[i])", exit_repair_start)
        truncate = source.index(
            "plannedRight.resize(static_cast<size_t>(edgeCount))", collect)
        smooth = source.index("repair0 = smoothLine(repair0)", truncate)
        append = source.index("plannedRight.push_back(point)", smooth)
